...
export function useAuctionSocket(roomId, userId, initialStatus) {
  ...
  useEffect(() => {
    ...
    s.on('skip_votes_updated', setSkipInfo);
  ...
  const skipVote = () => socket?.emit('skipVote', { roomId, userId });
  const withdrawBid = () => socket?.emit('withdrawBid', { roomId, userId });

  return { socket, participants, settings, auctionStatus, currentPlayer, timer, soldLog, squads, queue, skipInfo, updateSettings, startAuction, placeBid, pauseAuction, resumeAuction, endAuction, kickUser, closeRoom, skipVote, withdrawBid };
}
